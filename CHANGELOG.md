...
## 2026-08-08 — Course 2 (Practice) Units 7–9

- Authored Practice Unit 7 (Working with Buyers): the buyer consultation as the gate before showings, buyer agency duties and where buyers commonly ask agents to breach them, the prequalification/preapproval/loan-commitment distinction, showing conduct including tenant entry rights and the live visual inspection duty, an advising-boundaries table, and expectation management in both fast and slow markets. Worked examples cover a "preapproval" that was actually a prequalification and killed a deal after contingency removal, a ceiling stain the buyer wanted ignored, a confidentiality leak at an open house, and a tenant-occupied showing the seller could not authorize.
- Authored Practice Unit 8 (Writing & Negotiating Offers): what makes an offer strong beyond price, purchase agreement completion points, the real cost of waiving each contingency quantified in dollars, presenting offers via net comparison, multiple-offer mechanics, and the fair housing problem with buyer letters. Worked examples include a three-offer net comparison where the highest price is not the best offer, an appraisal gap computed to the buyer's actual cash requirement, a multiple-counteroffer failure binding a seller twice, and declining a buyer love letter with the explanation to give.
- Authored Practice Unit 9 (Contract-to-Close): building the timeline from acceptance and planning backward, active removal and the Notice to Perform procedure, inspections and repair request strategy, coordinating escrow/lender/title, and a table of escrow failure causes with early warning signs. Worked examples cover a full calendar build with backward planning, a listing agent wrongly advising cancellation on a passed deadline, a 34-item repair request that hardened the seller, and the disclosure obligation that survives a cancelled escrow.
- Question bank now 545 items, all validated. Contracts coverage has reached its blueprint target (12.5% vs 12%); agency and valuation remain under-weighted pending dedicated exam-level fill items.
